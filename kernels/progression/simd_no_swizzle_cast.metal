...
const device half* X2 = X + X_shape[1];
...
for(ushort k = 0; k < num_iters; k++) {
    ...
    for (ushort i = 0; i < PACKS_PER_THREAD; i++) {
        uint w = *W;
        thread half4 xs1 = *reinterpret_cast<const device half4*>(X);
        thread half4 xs2 = *reinterpret_cast<const device half4*>(X2);
        ...
        simdgroup_multiply_accumulate(sgMatrixR, sgMatrixW1, sgMatrixX1, sgMatrixR);
        ...
        simdgroup_multiply_accumulate(sgMatrixR, sgMatrixW2, sgMatrixX2, sgMatrixR);
        ...
        simdgroup_multiply_accumulate(sgMatrixR, sgMatrixW3, sgMatrixX3, sgMatrixR);
        ...
        simdgroup_multiply_accumulate(sgMatrixR, sgMatrixW4, sgMatrixX4, sgMatrixR);
...
if(remainder) {
    ...
    X -= (lu / 2) * (VALUES_PER_THREAD / 2);
    X2 -= (lu / 2) * (VALUES_PER_THREAD / 2);
    half s0 = *s;
    half b0 = *b;
...
simdgroup_store(sgMatrixDowncast, &result[8 * thread_position_in_grid.z], W_shape[0], 0, true);